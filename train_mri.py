...
def get_reference_videos(config, val_reference, indices, clamp_range=(None, None)):
    ...
    res_size = config.imagen.image_sizes[undex_index]
    # temp_down = config.imagen.temporal_downsample_factor[undex_index]  # not needed here because of interpolation of the samples later
    videos = [ zoom(v, (1, res_size/v.shape[1], res_size/v.shape[2], 1)).clip(*clamp_range) for v in videos ]
...
if __name__ == "__main__":
    ...
    encoder_infos=[] 
    unet_infos=[]
    for i, (k, v) in enumerate(config.encoders.items()):
    ...
    if trainer.is_main:
        ...
        temp = np.repeat(videos_ref, 3, axis=1)
        wandb.log({"videos": wandb.Video(temp, fps=2)}) #config.dataset.fps
        # reshape again for network 
